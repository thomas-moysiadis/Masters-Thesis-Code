...
a = ones(size(Y4,1),1);
Y1 = [Y1; a(length(Y1)+1:end,1)];
Y2 = [Y2; a(length(Y2)+1:end,1)];
Y3 = [Y3; a(length(Y3)+1:end,1)];
Y11 = [Y11; a(length(Y11)+1:end,1)];
Y = [Y1 Y2 Y3 Y4(:,2)/pi*180 Y5(:,2)/pi*180 Y6(:,2)/pi*180 Y7(:,2)/pi*180 Y8(:,2)/pi*180 Y9(:,2)/pi*180 Y10(:,2) Y11 Y12(:,2)/pi*180 Y13(:,2)/pi*180 Y14(:,2)/pi*180 Y15(:,2)/pi*180 Y16(:,2)/pi*180 Y17(:,2)/pi*180 Y18(:,2)/pi*180 Y19(:,2)/pi*180 Y20(:,2)/pi*180 Y21(:,2)/pi*180 Y22(:,2) -Y23(:,2) Y24(:,2)/pi*180 -Y25(:,2)/pi*180 Y26(:,2)/pi*180 Y27(:,2)/pi*180 Y28(:,2)/pi*180 Y29(:,2)/pi*180 -Y30(:,2)/pi*180 Y31(:,2)/pi*180 Y32(:,2)/pi*180 Y33(:,2)/pi*180];
T = Y4(:,1)';
tt = [0 3];
...
%3D Trajectory Plot
f16 = figure;
plot3(y(:,1)/pi*180, y(:,3)/pi*180, y(:,5)/pi*180, "b");
hold on
scatter3([0, 60], [0, 45], [0, 0], 30, "r", 'filled');
xlabel("θ (deg)");
ylabel("φ (deg)");
zlabel("ψ (deg)");
