...
xlabel("Time t (s)");
ylabel("Torque (Nm)");
title("Comparison of Constraints Torques for Sphere");
...
ylabel("Torque (Nm)");
ylim([-1 1]);
title("Comparison of Constraints Torques for Cylinder 1");
legend(["Tx (Python)", "Ty (Python)", "Tz (Python)", "Tx (MotionView)", "Ty (MotionView)", "Tz (MotionView)"]);
grid on
hold off
...
ylabel("Torque (Nm)");
ylim([-0.5 0.5]);
title("Comparison of Constraints Torques for Cylinder 2");
legend(["Tx (Python)", "Ty (Python)", "Tz (Python)", "Tx (MotionView)", "Ty (MotionView)", "Tz (MotionView)"]);
grid on
hold off
...
obj = VideoWriter("Animation_Single_Point_1.avi");
obj.FrameRate = 100;
open(obj);
f16 = figure;
for i = 1:length(t)
    ...
    hold on
    scatter3([0, 60], [0, 45], [0, 0], 100, "r", 'filled');
    xlabel("θ (deg)");
    ylabel("φ (deg)");
    zlabel("ψ (deg)");
    ...
    if i == 1
        imwrite(A, map, 'Animation_Single_Point_1.gif', DelayTime = 2.5/100, LoopCount = inf);
    else
        imwrite(A, map, 'Animation_Single_Point_1.gif', WriteMode = 'append', DelayTime = 2.5/100);
    end
...
obj = VideoWriter("Animation_Single_Point_2.avi");
obj.FrameRate = 100;
open(obj);
[XS,YS,ZS] = sphere;
XSYSZS = 32*[XS; YS; ZS];
...
Rx = [1 0 0; 0 cos(60/180*pi) sin(60/180*pi); 0 -sin(60/180*pi) cos(60/180*pi)];
Ry = [cos(45/180*pi) 0 -sin(45/180*pi); 0 1 0; sin(45/180*pi) 0 cos(45/180*pi)];
Rz = [cos(0) sin(0) 0; -sin(0) cos(0) 0; 0 0 1];
R = Rz*Ry*Rx;
R = R';
target_point = R*[0 0 R1*10^3+8]';
...
for i = 1:length(t)
    ...
    scatter3([0 target_point(1)], [0 target_point(2)], [0 target_point(3)], 150, 'k', 'filled');
    ...
    if i == 1
        imwrite(A, map, 'Animation_Single_Point_2.gif', DelayTime = 2.5/100, LoopCount = inf);
    else
        imwrite(A, map, 'Animation_Single_Point_2.gif', WriteMode = 'append', DelayTime = 2.5/100);
    end
    %exportgraphics(gca,'Animation_Single_Point_2.gif', 'Append', true);
    %saveas(gcf,['Frame_', num2str(i), '.png']);

    %if i == 1
    %    saveas(gcf,'Path_1_2.png');
    %end
