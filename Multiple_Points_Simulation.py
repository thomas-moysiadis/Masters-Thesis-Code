# -*- coding: utf-8 -*-
"""
Created on Fri Nov 21 00:27:17 2025
...
for i in range(0,5):
    ...
    q18 = m.Var(0)
    if i == 0 or i==4:
        q2 = m.Var(-R3/R1*np.pi/3)
        q4 = m.Var(-R2/R1*np.pi/6)
        q8 = m.Var(-np.pi/6)
        q10 = m.Var(np.pi/6)
        q14 = m.Var(-np.pi/3)
    elif i == 1 or i == 3:
        q2 = m.Var(2*R3/R1*np.pi/3)
        q4 = m.Var(0)
        q8 = m.Var(np.pi/3)
        q10 = m.Var(0)
        q14 = m.Var(2*np.pi/3)
    ...
    if i == 0:
        ...
        m.time = np.linspace(3,6,300)
    elif i == 2:
        m.time = np.linspace(6,9,300)
    elif i == 3:
        m.time = np.linspace(9,12,300)
    else:
        m.time = np.linspace(12,15,300)
    m.options.IMODE = 4
    m.options.NODES = 1
    ...
    if i == 0:
        ...
        plt.plot(m.time,np.array(q1)-np.pi/2,label='theta')
        plt.plot(m.time,np.array(q3)-np.pi/4,label='phi')
        plt.plot(m.time,q5.value,label='psi')
        plt.plot(m.time,np.array(q7)-np.pi/2,label='theta_r2')
        plt.plot(m.time,np.array(q9)+np.pi/2,label='theta_p2')
        plt.plot(m.time,q11.value,label='theta_r3')
        plt.plot(m.time,np.array(q13)-np.pi,label='theta_p3')
    elif i == 2:
        plt.plot(m.time,np.array(q1)+np.pi/2,label='theta')
        plt.plot(m.time,np.array(q3)-np.pi/4,label='phi')
        plt.plot(m.time,q5.value,label='psi')
    ...
    plt.legend(); plt.grid()
    plt.xlabel('Time')
    plt.savefig('data_points_'+str(i+1)+'.png',dpi=600)
    plt.show()
    ...
    if i == 0:
        ...
        df['q8'] = np.array(q8)[:-1]
        df['q9'] = np.array(q9)[:-1]
        df['q10'] = np.array(q10)[:-1]
        df['q11'] = np.array(q11)[:-1]
        df['q12'] = np.array(q12)[:-1]
        df['q13'] = np.array(q13)[:-1]
        df['q14'] = np.array(q14)[:-1]
        df['q15'] = np.array(q15)[:-1]
        df['q16'] = np.array(q16)[:-1]
        df['q17'] = np.array(q17)[:-1]
        df['q18'] = np.array(q18)[:-1]
        df.columns = range(df.shape[1])
        df.to_excel('data_points_1.xlsx', index=False)
    elif i == 1:
        df = pd.DataFrame()
        df['q1'] = np.array(q1)[:-1] - np.pi/2
        df['q2'] = np.array(q2)[:-1]
        df['q3'] = np.array(q3)[:-1] - np.pi/4
        df['q4'] = np.array(q4)[:-1]
        df['q5'] = np.array(q5)[:-1]
        df['q6'] = np.array(q6)[:-1]
        df['q7'] = np.array(q7)[:-1] - np.pi/2
        df['q8'] = np.array(q8)[:-1]
        df['q9'] = np.array(q9)[:-1] + np.pi/2
        df['q10'] = np.array(q10)[:-1]
        df['q11'] = np.array(q11)[:-1]
        df['q12'] = np.array(q12)[:-1]
        df['q13'] = np.array(q13)[:-1] - np.pi
        df['q14'] = np.array(q14)[:-1]
        df['q15'] = np.array(q15)[:-1]
        df['q16'] = np.array(q16)[:-1]
        df['q17'] = np.array(q17)[:-1]
        df['q18'] = np.array(q18)[:-1]
        df.columns = range(df.shape[1])
        df.to_excel('data_points_2.xlsx', index=False)
    elif i == 2:
        df = pd.DataFrame()
        df['q1'] = np.array(q1)[:-1] + np.pi/2
        df['q2'] = np.array(q2)[:-1]
        ...
        df['q18'] = np.array(q18)[:-1]
        df.columns = range(df.shape[1])
        df.to_excel('data_points_3.xlsx', index=False)
    elif i == 3:
        df = pd.DataFrame()
        df['q1'] = np.array(q1)[:-1] - np.pi/2
        df['q2'] = np.array(q2)[:-1]
        ...
        df['q18'] = np.array(q18)[:-1]
        df.columns = range(df.shape[1])
        df.to_excel('data_points_4.xlsx', index=False)
    else:
        df = pd.DataFrame()
        df['q1'] = np.array(q1) + np.pi/2
        df['q2'] = np.array(q2)
        df['q3'] = np.array(q3) + np.pi/4
        df['q4'] = np.array(q4)
        df['q5'] = np.array(q5)
        df['q6'] = np.array(q6)
        df['q7'] = np.array(q7) + np.pi/2
        df['q8'] = np.array(q8)
        df['q9'] = np.array(q9) - np.pi/2
        df['q10'] = np.array(q10)
        df['q11'] = np.array(q11)
        df['q12'] = np.array(q12)
        df['q13'] = np.array(q13) + np.pi
        df['q14'] = np.array(q14)
        df['q15'] = np.array(q15)
        df['q16'] = np.array(q16)
        df['q17'] = np.array(q17)
        df['q18'] = np.array(q18)
        df.columns = range(df.shape[1])
        df.to_excel('data_points_5.xlsx', index=False)
